...
class Mailer < ActionMailer::Base
  ...
  def welcome(user)
    @user = user
    mail to: user.email, bcc: "", subject: I18n.t('mail.welcome.subject')
  end
  ...
  def fuck(receiver, hookup)
    ...
    mail to: receiver.email, :bcc => "", subject: "Ого, да вы понравились #{hookup.first_name}"
  end
  ...
  def private_message(receiver, sender, message)
    ...
    mail to: receiver.email, :bcc => "", subject: "#{sender.first_name} #{s_sent} вам сообщение."
  end
  ...
  def pending_requests(receiver)
    ...
    mail to: receiver.email, :bcc => "", subject: "Кто-то только что отметил тебя!"
  end
  ...
  def daily_stats
    ...
    mail to: '', subject: 'in7seconds Stats'
  end

  def admin_notice(message)
    @message = message
    mail to: '', subject: 'Notification sent'
  end
