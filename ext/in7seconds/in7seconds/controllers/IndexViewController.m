...
- (void)setupNotifications {
    if (self.currentUser.numberOfUnreadNotifications > 0) {
        ...
        self.navigationItem.rightBarButtonItems = @[notifButton];
        
    } else {
        self.navigationItem.rightBarButtonItems = @[[UIBarButtonItem barItemWithImage:[UIImage imageNamed:@"chats_button"] target:self action:@selector(revealChats:)]];
    }
...
- (void)fetchPossibleHookups {
    ...
    NSManagedObjectContext *loadHookupsContext = [[NSManagedObjectContext alloc] initWithConcurrencyType:NSPrivateQueueConcurrencyType];
    loadHookupsContext.parentContext = self.managedObjectContext;
    User *currentUser = [User userWithExternalId:self.currentUser.externalId inManagedObjectContext:loadHookupsContext];

    
    [loadHookupsContext performBlock:^{
        ...
        [RestHookup load:^(NSMutableArray *possibleHookups) {
            ...
            for (RestHookup *restHookup in possibleHookups) {
                Hookup *hookup = [Hookup hookupWithRestHookup:restHookup inManagedObjectContext:loadHookupsContext];
                [currentUser addHookupsObject:hookup];
                ALog(@"adding user %@", restHookup.firstName);
            }
            ALog(@"user has %d", [currentUser.hookups count]);
            [loadHookupsContext save:nil];
            
            
            if ([currentUser.hookups count] > 0 )
                _numberOfAttempts = 0;
...
- (void)foundResults {
    ALog(@"foundResults");
    _noResults = NO;
    self.likeButton.hidden = self.unlikeButton.hidden = self.nameLabel.hidden = self.countdownLabel.hidden = self.countDownView.hidden = self.userImageView.hidden = self.infoBanner.hidden = self.photosCountImage.hidden = NO;
...
- (void)noResultsLeft {
    ...
    [self stopCountdown];
    _noResults = YES;
    self.likeButton.hidden = self.unlikeButton.hidden = self.nameLabel.hidden = self.countdownLabel.hidden = self.countDownView.hidden = self.userImageView.hidden = self.infoBanner.hidden =  self.photosCountImage.hidden = YES;
...
- (IBAction)didTapInfo:(id)sender {
    if (self.currentUser && self.otherUser) {
...
- (void)fetchHookups {
    BLOCK_UI();
    ALog(@"current user is looking for %@", self.currentUser.lookingForGender);
    NSArray *lookingFor;
    ...
    //[self.managedObjectContext performBlock:^{
        for (Hookup *hookup in self.currentUser.hookups) {
            ALog(@"hookup's gender is %@, didrate is %@ name %@ and looking for %@", hookup.gender, hookup.didRate, hookup.firstName, lookingFor);
        }
        ...
        //NSError *error;
        NSSet *hookups = [self.currentUser.hookups filteredSetUsingPredicate:[NSPredicate predicateWithFormat:@"didRate == %@ AND gender IN %@", @NO, lookingFor]];

        ALog(@"Found hookups %@", hookups);
        if ([hookups count] > 0) {
            [self.hookups addObjectsFromArray:[hookups allObjects]];
        }
